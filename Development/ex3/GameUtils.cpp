...
std::string GameUtils::GetTeamNameFromFileName(const std::string& path)
{
	size_t firstInd = path.rfind('/') + 1;
	size_t lastInd = path.find(".dll");

	return path.substr(firstInd, lastInd - firstInd);
}
